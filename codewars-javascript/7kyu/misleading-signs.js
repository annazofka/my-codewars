...
function gaslighting(shirtWord, yourWord, friendsLetters) {
  for (let i = 0; i < shirtWord.length; i++) {
    if (shirtWord[i] != yourWord[i]) {
      for (let x = 0; x < friendsLetters.length; x++) {
        if (
          shirtWord[i] === friendsLetters[x] ||
          yourWord[i] === friendsLetters[x]
        ) {
          return true;
        }
      }
...
// Best practice:
function gaslighting(shirtWord, yourWord, friendsLetters) {
  return [...shirtWord].some(
    (c, i) =>
      yourWord[i] !== c &&
      (friendsLetters.includes(c) || friendsLetters.includes(yourWord[i]))
  );
}
